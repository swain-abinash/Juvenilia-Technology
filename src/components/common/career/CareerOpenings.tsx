...
type CareerOpeningsProps = {
  ...
  onApply: (job: CareerPageData['openings'][0]) => void;
};

const JobListItem = ({ job, onApply }: { job: CareerPageData['openings'][0], onApply: (job: CareerPageData['openings'][0]) => void }) => {
        ...
        {isOpen && (
          ...
          >
            <div className="p-4 md:p-6 border-t border-gray-800/50">
              ...
              <button
                className="bg-gradient-to-r from-orange-400 via-orange-500 to-orange-600 text-white font-bold py-3 px-8 rounded-lg hover:shadow-lg hover:scale-105 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:ring-opacity-50"
                onClick={() => onApply(job)}
              >
                Apply Now
              </button>
...
export const CareerOpenings = ({ openings, onApply }: CareerOpeningsProps) => {
  return (
